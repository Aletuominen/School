...
private:
    ...
    // Seuraavat apufunktiot on toteutettu
    ...
    void printTallestOrShortest(Person* const &person, Person* const &tallest,
                                const std::string tallshort,
                                std::ostream& output) const;


    std::vector<Person*> getSiblings (Person* const &sibling) const;

    std::string howGrand(int n) const;

    IdSet grandRecursive(Person* person_ptr, int n, IdSet grands,
                         const bool &porc) const;

    Person*  heightRecursive(Person* const &person, Person* max_person,

...
CardSlot::CardSlot(CheckFunction function, QWidget *parent, bool stackHeight):
    QFrame(parent), topCard_(nullptr), checkFunction_(function), hideUnder(stackHeight)
{
...
void CardSlot::addCard(Card *card)
{
    if (topCard_ == nullptr){
        ...
        topCard_->stackCard(card, hideUnder);
    }
    topCard_= card;
    card->allowOpen();
}
